...
async def set_ads(client, message):
    try:
        ...
        if '#' not in command_args or len(command_args.split('#')) < 3:
            await message.reply_text(f"Usage: /set_ads {{ads name}}#{{time}}#{{photo URL}} <a href=https://t.me/hacker_x_official_777>Explain</a>")
            return
        ...
        await db.Spidey_set_ads_link(url)
...
async def del_ads(client, message):
    try:
        ...
        current_link = await db.Spidey_get_ads_link()
        if current_link:
            is_deleted = await db.Spidey_del_ads_link()
            if is_deleted:
