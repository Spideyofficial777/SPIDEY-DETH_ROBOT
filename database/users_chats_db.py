...
class Database:
    ...
    async def give_free_trial(self, user_id):
     ...
     # Spidey BOTS = @IM_Spidey
    async def Spidey_set_ads_link(self,link):
        await self.Spidey_ads_link.update_one({} , {'$set': {'link': link}} , upsert=True)
    async def Spidey_get_ads_link(self):
        link = await self.Spidey_ads_link.find_one({})
        if link is not None:
            return link.get("link")
        else:
            return None
            
    async def Spidey_del_ads_link(self):
        try: 
            isDeleted = await self.Spidey_ads_link.delete_one({})
